/* 한진 SMART 통합관제 — Figma get_design_context(React+Tailwind) → 순수 HTML/CSS 변환기
   ...
   손으로 고치지 말고 이 파일을 고쳐 `node src/posco/_gen/conv.js` 로 다시 돌린다. */
'use strict';
const fs = require('fs');
const path = require('path');

const GEN = __dirname;
const ART = path.join(GEN, '..');                 /* src/posco — 내려받은 에셋이 있는 곳 */
...
const SCREENS = [
  ...
  { key: '39-9942', node: '39:9942', px: 'pka', name: 'Screen/Control Main (Ack Dialog)', out: 'posco-ack', meta: ['meta-2-18441.xml', 'meta-39-9942.xml'] },
  { key: '40-10872', node: '40:10872', px: 'pko', name: 'Screen/Control Main (Overview Popup)', out: 'posco-overview', meta: ['meta-2-18441.xml', 'meta-40-10872.xml'] },
  { key: '1-2606', node: '1:2606', px: 'pkr', name: 'Screen/Access Route', out: 'posco-route', meta: ['meta-2-18441.xml', 'meta-1-2606.xml'] },
  { key: '1-2487', node: '1:2487', px: 'pkf', name: 'Screen/All Floors', out: 'posco-floors', meta: ['meta-2-18441.xml', 'meta-1-2487.xml'] },
  { key: '1-2400', node: '1:2400', px: 'pkd', name: 'Screen/Floor Detail', out: 'posco-detail', meta: ['meta-2-18441.xml', 'meta-1-2400.xml'] },
  /* SOP 대응 절차 STEP 1~3 — 스튜디오에서는 한 장면(sop)으로 합쳐 10초마다 넘긴다(posco-live.js installSop) */
  { key: '1-9688', node: '1:9688', px: 'pks1', name: 'Screen/Control Main (SOP Awareness)', out: 'posco-sop1', sop: true, meta: ['meta-2-18441.xml', 'meta-1-9688.xml'] },
  { key: '1-9770', node: '1:9770', px: 'pks2', name: 'Screen/Control Main (SOP Dissemination)', out: 'posco-sop2', sop: true, layer: 2, meta: ['meta-2-18441.xml', 'meta-1-9688.xml', 'meta-1-9770.xml'] },
  { key: '1-9852', node: '1:9852', px: 'pks3', name: 'Screen/Control Main (SOP Tracking)', out: 'posco-sop3', sop: true, layer: 3, meta: ['meta-2-18441.xml', 'meta-1-9688.xml', 'meta-1-9770.xml'] },
  /* SOP 체계도 — 대화상자 왼쪽 메뉴(Tab/Diagram)로 들어가는 두 번째 보기. 본문 · 메뉴만 겹쳐 쓴다 */
  { key: '1-9443', node: '1:9443', px: 'pks4', name: 'Screen/Control Main (SOP Diagram)', out: 'posco-sop4', sop: true, layer: 4, meta: ['meta-2-18441.xml', 'meta-1-9688.xml', 'meta-1-9770.xml', 'meta-1-9443.xml'] },
];
...
const INLINE_SVG = ['btn-ack-bg.svg'];
...
const GRAFT = {
  'posco-route': [
    { from: '1-2400', node: '1:1000', into: '1:2613', key: 'nav-overview', at: 0, show: true },
    { from: '1-2400', node: '1:1090', into: '1:2613', key: 'nav-route', at: 1, show: true },
  ],
};
...
const VARIANT_SETS = {
  /* SOP 화면 — 헤더 SOP Status(1:10011)는 STEP 01·02·03·end 네 상태, 상황종료 단추(1:10439)는 default·hover 두 상태.
     화면 dc 에는 한 상태만 실려 있어(컴포넌트 뿌리 id = 그 변형 id) 세트 원본(dc-1-10011 · dc-1-10439)으로 모든 상태를 심는다.
     라이브(sop-live.part.js)가 단계·마우스오버·상황종료에 맞춰 보이는 상태만 바꾼다. */
  '1:10012': {
    key: 'sop-status', src: 'dc-1-10011.txt', amap: '1-10011', comp: 'SopStatus', prop: 'property1',
    values: ['01', '02', '03', 'end'], active: '01',
  },
  '1:10440': {
    key: 'btn-end', src: 'dc-1-10439.txt', amap: '1-10439', comp: 'ButtonEndSituation', prop: 'property1',
    values: ['default', 'hover'], active: 'default',
  },
};
...
function liveStamp() {
  try {
    const p = path.join(GEN, '..', '..', 'posco-live.js');
...
for (const r of results) {
  ...
  const MODULE = [
    '/* POSCO KDB CCTV 관제 — ' + r.S.name,
    '   원본: Figma dj0SONcO5BySCm7yDdZrNc / node ' + r.S.node + ' (1920x1080, page Page 1)',
    ...
    '   생성기: src/posco/_gen/conv.js — 손으로 고치지 말 것. */',
    '(function () {',
    "  var A = 'src/posco/';",
  ...
  const PREVIEW = [
    ...
    '<script src="../posco-live.js?v=' + liveStamp() + '"></script>',
    '<script>if(new URLSearchParams(location.search).get("live")!=="0")window.initPosco && window.initPosco(document.querySelector(".' + PX + '-root"));</script>',
  ...
  fs.writeFileSync(path.join(STUDIO, 'src', 'posco', r.S.out.replace('posco-', '') + '.html'), PREVIEW);
