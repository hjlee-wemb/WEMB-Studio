/* 라이트 테마용 에셋 사본 만들기 — src/hana/*.svg → *-lt.svg
   ...
   실행: node src/hana/_gen/mk-light-assets.js */
'use strict';
const fs = require('fs');
...
function light(hex) {
  ...
  if (c.s < 0.3) return hsl2hex({ h: 168, s: 0.09, l: Math.max(0.06, Math.min(0.98, 1 - c.l * 0.9)) });
