...
스튜디오 템플릿 이름은 **`Dashboard: HANA Bank H.I.T`** (2D 대시보드 · 서비스·거래 · June 30, 2025).
...
- 템플릿 `Dashboard: HANA Bank H.I.T` → `tpl:'hana'`, 슬라이드 14장이 화면 14개(`tplScene` = 화면 키)로 만들어진다.
...
`patch-textbox-trim.js` 가 트림을 쓴 규칙마다 자식 글자에도 같은 트림을 거는 규칙을 함께 낸다.

**단 줄이 하나일 때만 건다**(`>p:first-of-type:last-of-type`).
trim-both 은 덩어리의 첫 줄 위와 마지막 줄 아래만 깎는 것인데, Figma 는 두 줄짜리 글자를
`<p>` 두 개로 쪼개 내므로 줄마다 걸면 **줄 간격이 통째로 사라져 글자가 서로 붙는다.**
처음엔 그걸 놓쳐서 네트워크현황의 `CD-AOC-`/`APML01` 라벨 69개가 겹쳐 버렸다
(줄 간격이 12px → 7.08px, 즉 글자 높이보다 좁아졌다).
첫 줄만 `trim-start`, 마지막 줄만 `trim-end` 로 나눠 걸어도 Pretendard 의 cap 지표 때문에
여전히 원본보다 좁게 나온다 — **여러 줄짜리는 아예 손대지 않는 게 원본과 같다.**
실측: 14화면의 쌓인 줄 라벨 106개 모두 줄 간격이 선언된 line-height 그대로다.
...
상면관리의 자산 상세 팝업(`#hnf-Popup/Asset Detail`)에 닫기를 붙였다 — `patch-popup-close.js`.

- 닫기 단추(`Button/Close`)를 누르면 살짝 작아지며 사라지고, 다 사라진 뒤 `display:none` 으로
  빠져 뒤에 있는 도면을 가리지 않는다(실측: 닫힌 뒤 팝업 자리를 찍으면 도면이 잡힌다).
- **`transform` 은 건드리지 않는다.** 원본이 가운데 정렬용 `translate(-50%,-50%)` 를 걸어 뒀다
  → CSS `scale`·`opacity` 속성으로만 여닫는다. 다시 열면 원래 자리로 정확히 돌아온다
  (`matrix(1,0,0,1,-400,-370)` 그대로).
- `Esc` 로도 닫힌다.
- **다시 열기** — 한 번 닫으면 되돌릴 길이 없으면 곤란하므로, 위치 트리의 층·방(`Node/Level *`)을
  누르면 돌아온다. 도면은 통짜 이미지라 랙을 따로 집을 수 없어 트리를 여는 손잡이로 삼았다.
- 패널편집 중에는 동작하지 않는다(`editing()`). 글자를 고치다 팝업이 닫히면 안 된다.
  실측: `.dtstage.dt-content-editing` 이 붙어 있는 동안 단추·Esc 모두 무반응, 풀면 다시 닫힌다.
- 규칙은 `[data-name^="Popup"]` 전체에 걸리므로 나중에 다른 화면에 팝업이 생겨도 그대로 듣는다
  (지금 팝업이 있는 화면은 상면관리뿐이다).

## 발생시각 되찍기 (2026-09-09)

이벤트 로그·이벤트 리스트의 발생시각을 **열 때마다** 최근으로 다시 찍는다 — `patch-restamp.js`.
시안에 박힌 값은 `2025-00-00 00:00:00` 자리표시자라, 그대로 두면 오지 않은 날짜를 가리켜
살아 움직이는 관제 화면이 아니라 오래 묵은 시안으로 보인다.

계산은 공용 `src/recent-time.js` 가 한다(줄 사이 간격·글자 모양을 원본 그대로 지킨다).
전부 같은 시각이면 자리표시자로 보고 위에서부터 45~210초씩 벌려 흐르는 표로 만든다.

### 어디를 만지고 어디를 안 만지나
14화면을 전수 확인한 결과 시각 글자는 **딱 네 곳**뿐이다.

| 자리 | 무엇 | 처리 |
| --- | --- | --- |
| `[data-name="Event Log"]` | 하단 이벤트 현황 바(9화면 × 4~5줄) | 다시 찍는다 |
| `[data-name^="Widget/Event Log"]` | 이벤트 로그 위젯(이벤트 25 · 상면 5 · 보안 6) | 다시 찍는다 |
| `[data-name="X Axis"]` | 꺾은선 x축 눈금(08:00 …) | **건드리지 않는다** |
| `[data-name="Timestamp"]` | 머리글 시계 | **건드리지 않는다** |

앞의 둘만 골라 넘기므로 뒤의 둘은 애초에 걸리지 않는다.

### 보안시스템은 한 칸에 시각과 내용이 같이 있다
`2025-00-00 00:00:00  [Warning] SERVICE ALERT: …` 꼴이라 `recent-time.js` 의 '칸 전체가 시각'
규칙에 걸리지 않았다. → 공용 쪽에 `opt.prefix` 를 더해 **앞머리 시각만** 갈아 끼우고 뒤엣말은
한 글자도 건드리지 않는다(두 칸 띄어쓰기까지 그대로).

### 함정 둘
- **자리(offsetParent)로 '보이는 줄'을 가리면 안 된다.** 스튜디오는 런처가 덮고 있는 동안
  화면을 그려서, 그때는 멀쩡한 줄까지 전부 '안 보임'으로 잡혀 **한 줄도 안 고쳐진다.**
  칸에서 판까지만 거슬러 올라가며 `hidden` 인지 본다(자리와 무관).
- **설치 자리 앵커는 두 줄로 잡는다.** `try { markHot(root); }` 는 `installVariants` 안에도
  똑같이 있어서, 한 줄만 보면 거기에 끼어들어 **변형을 바꿀 때마다 표가 다시 찍힌다.**

실측(14화면): 자리표시자 0개 · 맨 윗줄 0~2분 전 · 줄마다 내림차순 · x축 눈금과 머리글 시계 그대로 ·
다시 열면 값이 새로 찍힌다.

## 이벤트 리스트 단추 살리기 (2026-09-09)

`patch-event-list.js` — 두 가지 이벤트 리스트를 **같은 코드로** 다룬다.

| 자리 | 있는 것 |
| --- | --- |
| `[data-name="Event Log"]` (하단 바, 9화면) | 등급 칩 4 · 초기화 · 삭제 · 줄 4 |
| `[data-name^="Widget/Event Log"]` (이벤트현황 큰 표) | 등급 칩 4 · 초기화 · 조회 · 검색칸 2 · 줄 25 |

- **등급 칩**(심각·경계·주의·정상) — 누르면 그 등급만 남기고 나머지는 **흐려진다**. 다시 누르면 풀린다.
  지우지 않고 흐리는 이유는 걸러 낸 결과가 0줄일 때 표가 텅 비기 때문이다
  (한진·SK하이닉스 시안도 같은 방식이다). 고르지 않은 칩도 같이 흐려져 지금 무엇을 보는지 드러난다.
- **조회** — 업무그룹·호스트 칸의 글자가 든 줄만 남기고 흐린다. 칸이 비면 전부 보인다.
- **초기화** — 등급·검색을 풀고 **지웠던 줄까지 되살린다.**
- **삭제** — 골라 둔 줄을 스르르 지운다. 고른 줄이 없으면 아무 일도 하지 않는다(오작동 방지).
- **줄 고르기** — 줄을 누르면 골라진다. 원본에 `status=active` 상태가 있으니 설계에 있는 동작이다.
- **검색칸** — 눌러야 고쳐 쓸 수 있게 되고(`contenteditable` 을 그때 붙인다), Enter 로 조회한다.
  원래 글자(`search...`)는 안내문처럼 두었다가 처음 누르면 비우고, 빈 채로 나가면 되돌린다.
- 패널편집 중에는 **전부 쉰다** — 글자를 고치려다 줄이 지워지면 안 된다.

### 시안에 박혀 있던 '상태 본보기 줄'
Figma 는 hover·active 가 어떻게 보이는지 알려 주려고 **그 상태의 줄을 하나씩 끼워** 뒀다.
그대로 옮기면 늘 그 색으로 앉아 있어, 가만히 있는데도 한 줄은 눌린 것처럼 보인다.

| 화면 | hover 본보기 | active 본보기 |
| --- | --- | --- |
| 상면관리 | `#hnf-row_2` `rgba(35,77,72,.3)` | `.n1_2539` `rgba(0,145,120,.3)` |
| 이벤트현황 | `status=hover` | `status=active` |
| 보안시스템 | `rgba(35,77,72,.3)` | `rgba(2,108,96,.4)` (이 화면만 색이 다르다) |

13개 리스트 중 10개에 hover 본보기가, 4개에 active 본보기가 들어 있었다.

→ **본보기 줄은 쉴 때 보통 줄처럼 되돌리고, 그 겉모습을 진짜 hover·고름 상태에 쓴다.**
색을 새로 짓지 않고 **시안이 준 클래스를 갈아 끼우므로** 라이트 테마와 '색 정하기'가 저절로 따라온다
(실측: 다크 hover `rgba(35,77,72,.3)` → 라이트 `rgba(0,120,100,.12)`).

- 상태는 줄 바탕만이 아니라 **글자색까지** 바뀐다(보통 `#e4e4e4` · hover 청록 `#00c8a5` · active `#eff9f6`).
  글자색은 칸이 아니라 그 안의 `<p>` 에 걸려 있어서, 줄 속을 **차례대로 훑어 통째로** 갈아 끼운다.
  본보기 줄과 보통 줄은 구조가 같아 자리끼리 짝이 맞는다(개수가 다르면 아예 건드리지 않는다).
- hover 와 active 를 가르는 기준은 **바탕색의 채도**다 — 강조색(active)은 0.98~1.0, 은은한 청록(hover)은 0.55.
  이름이 있으면(`status=active`) 이름을 먼저 믿는다.

### 만들 때 짚은 것
- 단추에 `data-name` 이 없다(`#hne-1:4454` 처럼 노드 id 뿐) → **글자로 찾는다**(`초기화`·`조회`·`삭제`).
- **줄을 이름으로만 고르면 안 된다.** 상면관리의 active 본보기(`.n1_2539`)에는 `data-name` 이 없다.
  이름 붙은 줄을 기준 삼고, **칸 수가 같은 이름 없는 형제**까지 줄로 친다.
- **"칸이 여럿"으로 가르는 것도 안 된다.** 보안 로그는 한 줄이 칸 하나(긴 글 한 덩이)다.
  이 규칙으로 바꾸기 전에는 보안 두 화면이 통째로 빠져 있었다(13개 중 11개만 걸렸다).
- 등급은 줄 안에서 `심각|경계|주의|정상` **글자와 정확히 같은 잎 노드**를 찾아 읽는다
  (칸 순서에 기대지 않아 두 리스트에 그대로 듣는다).

실측 — **13개 리스트 전부**: 쉴 때 줄 겉모습 한 가지(본보기 사라짐) · hover·고름·복귀 모두 동작.
이벤트현황: 25줄 · 칩 4 · 단추 2 · 검색칸 2, `주의` 누르면 1줄만 남고 다시 누르면 25줄 복귀,
검색 `07:5` → 4줄, 초기화 → 25줄·안내문 복귀. 하단 바: 2줄 골라 삭제 → 2줄, 빈 삭제 → 그대로,
초기화 → 4줄 복귀. 편집 중에는 고르기·칩·삭제 **모두 무반응**, 편집을 풀면 정상.
접기 화살표·발생시각 되찍기·팝업 닫기·순위표는 그대로다.
...
node patch-config.js && node patch-branches.js && node patch-takeelement.js \
  ...
  && node patch-textbox-trim.js && node patch-iso-plate.js && node patch-nowrap-text.js \
  && node patch-preview-recent.js && node conv.js
...
node patch-port-live.js && node patch-live.js && node patch-live2.js \
  ...
  && node patch-cloud-rank.js && node patch-popup-close.js && node patch-restamp.js \
  && node patch-event-list.js
```
