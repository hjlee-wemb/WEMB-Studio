/* 한진 SMART 통합관제 — Figma get_design_context(React+Tailwind) → 순수 HTML/CSS 변환기
   ...
   손으로 고치지 말고 이 파일을 고쳐 `node src/lselectric/_gen/conv.js` 로 다시 돌린다. */
'use strict';
const fs = require('fs');
const path = require('path');

const GEN = __dirname;
const ART = path.join(GEN, '..');                 /* src/lselectric — 내려받은 에셋이 있는 곳 */
...
const SCREENS = [
  ...
  { key: '76-597', node: '76:597', px: 'lsa', name: 'Popup/ACB - Diagnostics (Stage 1)', out: 'lselectric-acb', page: 'acb', meta: ['meta-page-2.xml'], layer: 1,
...
function findNode(node, nid) {
  ...
  const want = 'node-' + nid.replace(':', '_');
  if (typeof a.id === 'string' && a.id === want) return node;
  /* 컴포넌트에서 펼쳐진 노드는 id 가 **식**이다(`id={isChartOn ? "node-1_1000" : …}`) —
     출력할 때와 같은 방법으로 그 자리의 scope 로 풀어서 견준다. 안 풀면 나브 타일처럼
     컴포넌트로 그려진 노드를 영영 못 찾는다(떠다 심기가 여기서 막혔다). */
  if (a.id && typeof a.id === 'object' && a.id.expr) {
    try { if (String(evalExpr(a.id.expr, node.scope)) === want) return node; } catch (e) {}
  }
...
function styleFor(classes) {
  ...
  for (const c of classes) {
    ...
    switch (c) {
      ...
      case 'content-start': set('align-content', 'flex-start'); continue;   /* SOP 칩 묶음(flex-wrap) */
      ...
      case 'left-px': set('left', '1px'); continue;
      case 'top-px': set('top', '1px'); continue;
      ...
      case 'border-10': bAll = '10px'; continue;   /* 단선도 카드 판(65:2105)의 두꺼운 안쪽 테두리 */
      ...
      case 'rotate-1': tr.push('rotate(1deg)'); continue;
      case '-rotate-135': tr.push('rotate(-135deg)'); continue;              /* SOP 체계도 화살표 */                   /* SOP 화면 층 목록(원격) */
      ...
      case 'top-1/4': set('top', '25%'); continue;
      case 'left-1/4': set('left', '25%'); continue;
      case 'right-1/4': set('right', '25%'); continue;
      case 'inset-1/4': set('inset', '25%'); continue;
      case 'from-white': gFrom = '#ffffff'; continue;
      case 'to-white': gTo = '#ffffff'; continue;
      case 'from-black': gFrom = '#000000'; continue;
      ...
      case 'leading-none': set('line-height', '1'); continue;
      case 'bg-top-left': set('background-position', 'top left'); continue;
      case 'bg-top': set('background-position', 'top'); continue;
...
function buildScreen(S) {
  ...
  (GRAFT[S.out] || []).forEach((g) => {
    ...
    cut.__graft = g.key;
    cut.__graftShow = !!g.show;                    /* 처음부터 보이게 심는가(감춰 두고 JS 가 펴는 것이 기본) */
    /* 자리를 주면 그 자리에 끼운다 — 나브처럼 **순서가 뜻을 갖는** 줄에 붙일 때 필요하다
       (다른 화면이 종합현황 · 출입동선 · 시점 순서라 route 도 그 순서로 맞춘다). */
    if (typeof g.at === 'number') host.children.splice(g.at, 0, cut);
    else host.children.push(cut);
  });

  /* ── HTML/CSS 출력 ── */
  const PX = S.px, ROOT = '.' + PX + '-root';
  const rules = [], darkRules = [], out = [];
  /* 판 텍스처(body.png)를 깐 노드 — 라이트에서 흰 막을 덮을 자리(light.js 의 lightExtra).
     예전엔 메인 화면의 노드 클래스를 손으로 적어 두었는데, 화면이 늘자 나머지 판이 까맣게 남았다. */
  const textured = [];
  ...
  function walk(node, depth, parentBorder, parentOnDark, parentFont) {
    ...
    /* SOP STEP 2·3 은 STEP 1 화면(.pks1-root) **안에** 겹쳐 얹힌다 — 클래스 이름이 같으면 STEP 1 의 규칙이
       겹친 층까지 먹는다(익명 x97 이 서로 다른 뜻이라 '조치사항 :' 의 콜론이 사라지고 배지 그림이 틀어졌다).
       층 화면은 이름을 따로 짓는다. 모양은 라이브의 swapNodeClass 가 알아보는 n…/x숫자 꼴을 지킨다. */
    if (S.layer) uc = nid ? 'nS' + S.layer + '_' + nid.replace(/[^\w]/g, '_') : 'x' + (S.layer * 100000 + anon);
    ...
    if (a.style) {
      ...
      for (const k of Object.keys(obj)) {
        ...
        /* 에셋을 가리키는 url() 에는 에셋 폴더({{B}})를 붙인다 — mask 뿐 아니라 배경도.
           data:/http 로 시작하는 것(인라인 그라디언트 SVG 등)은 그대로 둔다. */
        if (BG_URL_PROPS.test(prop)) val = val.replace(/url\("([^"]+)"\)/g, (mm, f) => {
          if (/^(data:|https?:|\{\{B\}\})/.test(f)) return mm;
          usedAssets.add(f); return 'url("{{B}}' + f + '")';
        });
    ...
    /* translate(-50%) 가 반 픽셀에 떨어지면(계통·에너지 팝업 Device Info 161px → -80.5px) 합성 층이 반 픽셀에 그려져
       표 줄 테두리가 1px 번지고 밀린다. 높이·폭을 메타데이터로 알면 같은 이동을 margin 으로 바꾼다 — 레이아웃은 픽셀에 붙는다.
       (자리는 같다: top/left 기준 -h/2 · -w/2) */
    {
      const ti = decls.findIndex((x) => x[0] === 'transform');
      const parts = ti >= 0 ? decls[ti][1].split(/\s+(?=[a-zA-Z]+\()/) : [];
      /* 절대배치 · 이동만 있는 노드에 한한다(회전이 섞이면 메타 상자가 회전 뒤 상자라 쓸 수 없다) */
      if (ti >= 0 && META[nid] && decls.some((x) => x[0] === 'position' && x[1] === 'absolute')
          && parts.every((p) => /^translate[XY]\(-50%\)$/.test(p))) {
        const keep = parts.filter((p) => {
          if (p === 'translateY(-50%)') { decls.push(['margin-top', -(META[nid].h / 2) + 'px']); return false; }
          if (p === 'translateX(-50%)') { decls.push(['margin-left', -(META[nid].w / 2) + 'px']); return false; }
          return true;
        });
        if (keep.length !== parts.length) { if (keep.length) decls[ti][1] = keep.join(' '); else decls.splice(ti, 1); }
      }
    }
    ...
    if ((node.tag === 'p' || (META[nid] && META[nid].tag === 'text')) && META[nid] && META[nid].w != null) {
      ...
      /* SOP: 좌우 inset 으로 폭이 정해진 글자(체계도 노드 라벨 · text-center)에 폭을 박으면 right 가 무시돼 왼쪽으로 쏠린다 */
      const insetBox = !!S.sop && decls.some((x) => x[0] === 'inset' && String(x[1]).trim().split(/\s+/).length >= 2);
      if (!has('width') && !insetBox) decls.push(['width', +mw.toFixed(3) + 'px']);
    ...
    /* SOP 대화상자 안의 테두리는 전부 **안쪽 링**으로 — patch-sop-thin.js ①
       이 대화상자는 Figma stroke 가 오토레이아웃 크기에 안 들어간다(카드 127 = 14+99+14, 칩 133 = 12+8+10+91+12).
       CSS border 로 두면 카드마다 두께만큼 커져 세 카드가 10~16px 씩 밀리고, 0.5px 칩 테두리는 1px 로 올려 그려져
       칩 묶음(436px 꽉 참)이 한 줄 더 떨어졌다. 배경 화면(메인과 같은 조각)은 검증된 그대로 둔다. */
    const thinW = bwI >= 0 ? parseFloat(decls[bwI][1]) : 0;
    const thin = !!S.sop && inSopDialog && !hasSide && thinW > 0;
    if (bwI >= 0 && !hasSide && parseFloat(decls[bwI][1]) > 0 && (outside || thin || RING_STROKE.indexOf(ringKey) >= 0)) {
      ...
      const off = thin ? 0 : outside ? bw : +(bw / 2).toFixed(4);   /* 바깥 정렬은 통째로 밖, 가운데 정렬은 절반만 밖 */
      let rad = 'inherit';
    ...
    if (decls.some(([, v]) => /body\.png/.test(String(v)))) {
      /* 이미 자리를 잡은 노드(absolute 등)에 position:relative 를 덧씌우면 자리가 통째로 무너진다
         (라이트에서 자산정보현황 판이 왼쪽 끝으로 튀어나갔다) → 그 여부를 함께 넘긴다. */
      textured.push({ cls: '.' + uc, positioned: decls.some(([p, v]) => p === 'position' && v !== 'static') });
    }
    ...
    /* 다른 화면에서 옮겨 심은 조각 — 기본은 감춰 두고(라이브가 편다), show 면 처음부터 보인다 */
    if (node.__graft) {
      attrs.push('data-pk-graft="' + node.__graft + '"');
      if (!node.__graftShow) attrs.push('hidden');
    }
    if (node.tag === 'img') {
    ...
    /* SOP: span 끼리는 줄바꿈 없이 — patch-sop-thin.js ② */
    if (S.sop && node.tag === 'p' && node.children.length > 1 && node.children.every((c) => c.tag === 'span' || (c.tag === '#text' && !c.text.trim()))) {
      ...
      /* 이 p 는 글꼴 크기 0 · 줄높이 0 이고 줄상자는 span 이 만든다 — span 의 기준선 맞춤 탓에 상자가 18px 로
         부풀어 줄마다 4px 씩 밀렸다(Figma 상자 14px). 상자 높이를 Figma 값으로 박고 span 은 위에 붙인다. */
      if (META[nid] && META[nid].h) {
        rules.push(ROOT + ' .' + uc + '{height:' + +META[nid].h.toFixed(3) + 'px}');
        rules.push(ROOT + ' .' + uc + '>span{vertical-align:top}');
      }
      return;
    }
    ...
    const prevDlg = inSopDialog;
    if (dn === 'Dialog/SOP') inSopDialog = true;
...
let inSopDialog = false;   /* 지금 SOP 대화상자 안을 그리는가(patch-sop-thin ①) */
...
/* 다른 화면에서 떠다 심는 조각.
   출입동선(1:2606)만 왼쪽 나브가 '부지이동 · 전체보기'뿐이라 **다른 화면으로 넘어갈 자리가 없다**
   (나머지 다섯 장은 종합현황 · 출입동선을 갖고 있다). 그래서 단층(1:2400)의 나브 타일 둘을
   **원본 그대로 잘라다** 같은 줄 앞에 끼운다 — 손으로 그리지 않으므로 그림 · 글자 · 크기가 원본과 같다
   (둘 다 40x58, 줄은 flex gap 9 라 자리만 잡으면 그대로 붙는다).
...
/* Figma 의 stroke 정렬(INSIDE·CENTER·OUTSIDE)은 design context 에 나오지 않는다 — CSS border 는 늘 안쪽이라
   **가운데 정렬**이면 선의 절반이 상자 밖으로 나가 자리가 어긋난다(단선도 판 10px 는 5px 씩, 실측으로 잡았다).
   use_figma 로 정렬·굵기·색을 읽어 여기 적어 두고, 테두리는 투명으로 두고 같은 굵기의 outline 을
   절반만 안으로 밀어 그린다(둥근 모서리도 따라간다. 상자 크기는 그대로라 안쪽 배치가 안 흔들린다). */
const CENTER_STROKE = {
  '76-597': [
    { id: '65:2105', w: 10, c: '#121829' },                     /* Widget/Single Line Diagram > Card BG */
    { id: '65:2167', w: 1, c: '#2861ff' }, { id: '65:2168', w: 1, c: '#2861ff' }, { id: '65:2173', w: 1, c: '#2861ff' },
    { id: '65:2185', w: 1, c: '#2861ff' }, { id: '65:2189', w: 1, c: '#2861ff' }, { id: '65:2193', w: 1, c: '#2861ff' },
    { id: '70:2674', w: 1, c: '#273458' },                      /* Legend */
  ],
  /* 계통 · 에너지 팝업 — 왼쪽 판은 76:597 복사본이라 같은 선이 다른 id 로 있다. 범례 판도 모두 CENTER(use_figma 로 읽음) */
  '76-875': [
    { id: '76:935', w: 10, c: '#121829' },
    { id: '76:937', w: 1, c: '#2861ff' }, { id: '76:941', w: 1, c: '#2861ff' }, { id: '76:945', w: 1, c: '#2861ff' },
    { id: '76:950', w: 1, c: '#2861ff' }, { id: '76:954', w: 1, c: '#2861ff' }, { id: '76:958', w: 1, c: '#2861ff' },
    { id: '88:1797', w: 1, c: '#273458' }, { id: '88:1879', w: 1, c: '#273458' }, { id: '88:1939', w: 1, c: '#273458' },
  ],
  '94-5015': [
    { id: '94:5074', w: 10, c: '#121829' },
    { id: '94:5076', w: 1, c: '#2861ff' }, { id: '94:5080', w: 1, c: '#2861ff' }, { id: '94:5084', w: 1, c: '#2861ff' },
    { id: '94:5089', w: 1, c: '#2861ff' }, { id: '94:5093', w: 1, c: '#2861ff' }, { id: '94:5097', w: 1, c: '#2861ff' },
    { id: '94:5445', w: 1, c: '#273458' }, { id: '94:5176', w: 1, c: '#273458' }, { id: '94:5282', w: 1, c: '#273458' },
    { id: '94:5336', w: 1, c: '#273458' }, { id: '94:5229', w: 1, c: '#273458' },
  ],
};
function centerStrokeCss(S) {
  const list = CENTER_STROKE[S.key] || [];
  const out = [];
  for (const x of list) {
    const sel = '.' + S.px + '-root [data-node-id="' + x.id + '"]';
    out.push(sel + '{border-color:transparent!important;outline:' + x.w + 'px solid ' + x.c + ';outline-offset:-' + (x.w / 2) + 'px;}');
    const lt = LIGHT.lightDecl('outline-color', x.c, x.id);
    if (lt) out.push('.' + S.px + '-root[data-theme="light"] [data-node-id="' + x.id + '"]{outline-color:' + lt + ';}');
  }
  return out.join('\n');
}

const BG_URL_PROPS = /^(mask|background|border-image|list-style-image)/;
/* src/posco-live.js 의 '판 번호' — 내용이 바뀌면 값도 바뀐다(미리보기 주소의 ?v=). */
function liveStamp() {
  try {
    const p = path.join(GEN, '..', '..', 'lselectric-live.js');
...
const OVERLAY_CSS = (S) => [
  ...
  '.' + S.px + '-root{position:absolute;inset:0;background:transparent;z-index:auto;overflow:visible;}',
].join('\n');
...
for (const r of results) {
  ...
  const NODE_RULES = NODE_CSS.map(function (x) { return x.split('{{PX}}').join(PX); }).join('\n');
  const CSS = CSS0 + '\n' + NODE_RULES + '\n' + centerStrokeCss(r.S) + (r.S.overlay ? '\n' + OVERLAY_CSS(r.S) : '');
  const LIGHT_CSS = ['/* ' + r.S.name + ' — 라이트 테마(형상 동일, 색만 반전). 생성기: _gen/light.js */']
    .concat(LIGHT.lightExtra(PX, r.textured)).concat(r.darkRules).join('\n');
  const G = PX.toUpperCase();
  const MODULE = [
    '/* LS Electric STATCOM — ' + r.S.name,
    ...
    '   생성기: src/lselectric/_gen/conv.js — 손으로 고치지 말 것. */',
    '(function () {',
    "  var A = 'src/lselectric/';",
  ...
  /* 겹치는 화면 미리보기 — 바탕 화면 모듈을 함께 싣고 합치는 함수로 그린다(스튜디오와 같은 길) */
  ...
  const PREVIEW = [
    ...
    /* 라이브 레이어는 자주 다시 만든다 — 파일이 바뀌면 주소도 바뀌게 해야 브라우저가 옛것을 안 문다
       (README 함정 ⑦. 미리보기에서 '고친 게 안 먹네' 하고 한참 헤맸다). */
    '<script src="../lselectric-live.js?v=' + liveStamp() + '"></script>',
    ...
    '<script>(function(){var r=document.querySelector(".' + RPX + '-root");'
